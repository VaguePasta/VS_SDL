...
SDL_FPoint PlayerRelativeElemental(SDL_FPoint ElementalCenter)
{
...
void ElementalPathfinding(Elementals* elemental)
{
	...
	SDL_FPoint PlayerCenter = PlayerRelativeElemental(ElementalCenter);
...
void ElementalAttack(Elementals* elemental)
{
	if (elemental->isDodging || elemental->isDefending) return;
	SDL_FPoint ElementalCenter = { elemental->hitbox.x + elemental->hitbox.w / 2,elemental->hitbox.y + elemental->hitbox.h / 2 };
	elemental->ElementalCenter = ElementalCenter;
	SDL_FPoint PlayerCenter = PlayerRelativeElemental(ElementalCenter);
	SDL_FPoint PlayerRelativePosition = { PlayerCenter.x - 50,PlayerCenter.y - 50 };
	...
	{
		Elemental_isAttacking(elemental, ElementalCenter, PlayerCenter);
		if (elemental->CurrentSprite >= elemental->NumOfSprites - 1)
		{
...
{
	if (elemental->isHurt || elemental->isAttacking || elemental->isDodging || elemental->DodgeCooldown.GetTime() < 2500) return;
